const { MatchModel } = require("../models/DisplayTournementModels/Match");
const {
  ParticipantModel,
} = require("../models/DisplayTournementModels/Participant");
const { RoundModel } = require("../models/DisplayTournementModels/Round");
const {
  DisplayTournementModel,
} = require("../models/DisplayTournementModels/DisplayTournement");
...
const assignTeam = async (req, res) => {
  ...
  let foundOne = await DisplayTournementModel.findOne({
    name: tournementName,
  });
...
const CreateTournementMatches = async (req, res) => {
  ...
  for (let i = 0; i < rounds; i++) {
    ...
    for (let j = 0; j < currentMatchCount; j++) {
      ...
      for (let k = 2; k > 0; k--) {
        const savedParticipant = await new ParticipantModel({}).save();
  ...
  const newTournement = new DisplayTournementModel({
    name: tournementName,
    currentRound: 1,
    roundHistory: rd,
  });
...
const getTournement = async (req, res) => {
  ...
  const foundTournement = await DisplayTournementModel.findOne({
    name: tournementName,
  });
